...
- [ ] Git 全历史 Secret Scan 完成并处理历史泄漏
- [ ] 删除个人资料与敏感测试数据
- [ ] 复核密宗 restricted/sealed、灌顶和口传边界
- [ ] 复核知识库全文、译本、图片和注疏版权
- [x] 生成第三方依赖清单和机器可读 SBOM
- [ ] 分别决定代码、规则数据、知识内容与测试数据许可证
...
许可证建议仍是待决定项：可分别评估宽松代码许可证、不可自动附带授权的规则数据
条款、知识内容逐项授权和完全虚构测试数据声明。不得因仓库中存在依赖许可证而推定
三际观自身已经授权公开。
