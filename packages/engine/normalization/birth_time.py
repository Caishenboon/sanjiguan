...
This module deliberately does not calculate pillars, day boundaries, or charts.
"""
...
def _double_hour_bucket(value: datetime) -> int:
    """Candidate two-hour bucket beginning at 23:00; not a day-pillar rule."""
...
def _crosses_instant(start: datetime, end: datetime, instant: datetime) -> bool:
    ...
    return low <= instant_utc <= high


def equation_of_time_noaa(utc_value: datetime) -> float:
    """NOAA fractional-year approximation, returning minutes."""
...
def normalize_birth_time(
    ...
    if record.local_time is None:
        return BirthTimeNormalizationResult(
            original=record,
    ...
    civil_naive = datetime.combine(record.local_date, record.local_time)
    ...
    longitude_correction = 4 * (record.place.longitude - standard_meridian)
    ...
    candidates = [
        TimeCandidate(
            candidate_id="civil-time",
            basis="civil",
            local_datetime=civil_naive,
            utc_datetime=utc_value,
            method_id="TIME.CIVIL.IANA.V1",
        ),
        TimeCandidate(
            candidate_id="local-mean-solar-time",
            basis="local_mean_solar",
            local_datetime=mean_naive,
            utc_datetime=utc_value,
            method_id="TIME.SOLAR.MEAN.USNO.V1",
        ),
        TimeCandidate(
            candidate_id="local-apparent-solar-time",
            basis="local_apparent_solar",
            local_datetime=apparent_naive,
            utc_datetime=utc_value,
    ...
    return BirthTimeNormalizationResult(
        ...
        correction_chain=[
            CorrectionStep(
                ...
                offset_minutes=round(-dst_offset.total_seconds() / 60 + longitude_correction, 6),
