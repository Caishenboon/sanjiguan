...
export default function LiuxiangReadiness(){
 ...
 return <ProductShell title="六象合参" eyebrow="合参 · 六象" status="research only">
  ...
  {run&&<section aria-label="六象合参结果">
   <VerdictBanner status={run.status}><p>{run.status==="insufficient"?"六象结构已建立，但独立证契尚不足。补齐下列缺口后可重新起卷。":run.status==="contested"?"两象各有证契，主次尚未定；请同时查看各自逆证与缺口。":"主象已经显出，但研究规则仍未经审校，不能作为传统共识。"}</p></VerdictBanner>
   <MetricPair strengthBp={run.strength_bp} confidenceBp={run.confidence_bp}/>
   <div className="liuxiang-result-grid">{run.candidates.map((item,index)=><article className="liuxiang-card" data-primary={index===0} key={item.dimension_id}><header><h3>{labels[item.dimension_id]}</h3><span>{index===0?"主象候选":`第 ${index+1} 象`}</span></header><p><b>{productStatus(item.status)}</b></p><MetricPair strengthBp={item.strength_bp} confidenceBp={item.confidence_bp}/><p>证契 {item.support_count} 组 · 逆证 {item.counterevidence_count} 组</p>{item.missing_facts.length>0&&<p><b>尚缺：</b>{item.missing_facts.map(v=>missingLabels[v]||"未解释的资料缺口").join("、")}</p>}</article>)}</div>
