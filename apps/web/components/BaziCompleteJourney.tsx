...
export default function BaziCompleteJourney(){
 ...
 return <ProductShell title="八字传统结构" eyebrow="合参 · 八字" status="research_active · UNCONFIRMED"><aside className="privacy-note"><b>固定研究 Profile，不代表所有八字流派共识</b><p>结构与候选由确定性引擎生成；DeepSeek 不参与。旺衰、格局、调候和用神仍是该 Profile 的研究候选。</p></aside><section className="product-form"><label>传统性别参数<select value={sex} onChange={e=>setSex(e.target.value)}><option value="male">男</option><option value="female">女</option></select></label><button className="product-button" disabled={state==="running"} onClick={execute}>{state==="running"?"正在计算…":"执行八字研究 Profile"}</button>{error&&<p role="alert" className="field-error">{error}</p>}</section>{system&&<><section className="scope-card"><h2>四柱与传统结构</h2><p>{Object.values(mechanical?.pillars||{}).join("　")}</p><p>象势强度 {system.strength_bp/100}；证据可信度 {system.confidence_bp/100}；状态 {system.status}</p><h3>旺衰证据</h3><p>{result.strength?.status} · balance {result.strength?.balance_bp}</p><h3>格局与用神候选</h3><p>{result.pattern?.candidate} · {result.useful_elements?.candidates?.join("、")}</p></section><TechnicalDetails><pre>{JSON.stringify({profile:system.profile_id,ruleset:system.ruleset_version,mechanical,result,output_hash:run?.result.output_hash,trace_hash:run?.result.trace_hash},null,2)}</pre></TechnicalDetails></>}</ProductShell>;
}
