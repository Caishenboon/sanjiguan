...
export default function BaziCompleteJourney(){
 ...
 const pillars=Object.entries(mechanical?.pillars||{}).map(([label,value])=>({label:{year:"年柱",month:"月柱",day:"日柱",hour:"时柱"}[label]||label,value:String(value)}));
 return <ProductShell title="八字传统结构" eyebrow="合参 · 八字" status="研究中 · 尚未审校"><aside className="privacy-note"><b>固定研究方法，不代表所有八字流派共识</b><p>结构与候选由确定性引擎生成；DeepSeek 不参与。旺衰、格局、调候和用神仍是该方法下的研究候选。</p></aside><section className="product-form"><label>传统性别参数<select value={sex} onChange={e=>setSex(e.target.value)}><option value="male">男</option><option value="female">女</option></select></label>{state==="running"&&<RitualProgress steps={["时刻校正","节气定界","四柱成盘"]} current={1}/>}<button className="product-button" disabled={state==="running"} onClick={execute}>{state==="running"?"正在计算…":"按当前研究方法排盘"}</button>{error&&<p role="alert" className="field-error">{error}</p>}</section>{system&&<><section className="result-stage"><ObservationInstrument mode="bazi" title="四柱结构" items={pillars} caption="四柱按当前方法 Profile 形成；结构图不表示旺衰、格局或吉凶。"/><div className="scope-card"><p className="eyebrow">主要结构</p><h2>四柱与传统结构</h2><p className="structure-callout">{Object.values(mechanical?.pillars||{}).join("　")}</p><MetricPair strengthBp={system.strength_bp} confidenceBp={system.confidence_bp}/><p>当前状态：{system.status}</p><h3>旺衰研究证据</h3><p>{result.strength?.status} · 平衡基点 {result.strength?.balance_bp}</p><h3>格局与用神研究候选</h3><p>{result.pattern?.candidate} · {result.useful_elements?.candidates?.join("、")}</p></div></section><TechnicalDetails><pre>{JSON.stringify({profile:system.profile_id,ruleset:system.ruleset_version,mechanical,result,output_hash:run?.result.output_hash,trace_hash:run?.result.trace_hash},null,2)}</pre></TechnicalDetails></>}</ProductShell>;
}
